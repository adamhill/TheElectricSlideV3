...
struct PositionCalculationTests {
    ...
    @Suite("Absolute Position Tests")
    struct AbsolutePositionTests {
        private let cScale = StandardScales.cScale(length: 250.0)
        private let lScale = StandardScales.lScale(length: 250.0)
    ...
    @Suite("Angular Position Tests for Circular Scales")
    struct AngularPositionTests {
        private let circularCScale = ScaleDefinition(
            name: "C-Circular",
            function: LogarithmicFunction(),
            beginValue: 1,
    ...
    @Suite("Inverse Position Lookup Tests")
    struct InversePositionLookupTests {
        private let cScale = StandardScales.cScale(length: 250.0)
    ...
    @Suite("Circular Scale Inverse Lookups")
    struct CircularScaleInverseLookups {
        private let circularCScale = ScaleDefinition(
            name: "C-Circular",
            function: LogarithmicFunction(),
            beginValue: 1,
    ...
    @Suite("Round-Trip Accuracy Tests")
    struct RoundTripAccuracyTests {
        private let cScale = StandardScales.cScale(length: 250.0)
        private let aScale = StandardScales.aScale(length: 250.0)
        private let kScale = StandardScales.kScale(length: 250.0)
        private let circularCScale = ScaleDefinition(
