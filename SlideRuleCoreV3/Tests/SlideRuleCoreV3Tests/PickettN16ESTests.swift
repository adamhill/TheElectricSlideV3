...
struct PickettN16ESTests {
    ...
    func testResonantFrequency() async throws {
        ...
        let expected = 711.17
        let tolerance = 1.0 // 1 Hz tolerance (slide rule precision)
    ...
    func testFrequencyFunction() async throws {
        ...
        // Test that 12 cycles properly normalizes the frequency range
        let f1 = 1.0    // 1 Hz (10^0)
        let f2 = 1e12   // 1 THz (10^12)
        
        let pos1 = function.transform(f1)
        let pos2 = function.transform(f2)
        
        // log10(1) = 0, so 0/12 = 0
        // log10(1e12) = 12, so 12/12 = 1.0
